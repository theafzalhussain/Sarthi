...
> **Last updated:** August 2026 · After Phase 2 + 5 LLM providers
...
## 5. LLM PROVIDERS — 5 hain (commits `de67c96`, `fcf78d0`, `1c5946f`)

| Provider | Model (default) | Strength | Vision | Key kahan se |
|---|---|---|---|---|
| **groq** | `openai/gpt-oss-20b` | Sabse **tez** | ❌ | console.groq.com |
| **nvidia** | `nvidia/nemotron-3-ultra-550b-a55b` | Sabse **smart** (agentic) | ❌ | build.nvidia.com |
| **bluesminds** | `gpt-4o` | GPT-5.6/4o/GLM-5.2 gateway | ✅ | api.bluesminds.com |
...
**Order:** `groq → nvidia → bluesminds → openrouter → gemini`
`.env` mein `SAARTHI_PROVIDER_ORDER=nvidia,groq,...` se badal sakte hain.

**5 free tiers = practically unlimited.** Ek ki limit khatam ho to agla automatic.
...
### ✅ Verified (sandbox mein)
- 42 Python files, ~12,000 lines, **saare modules Python 3.9 pe import hote hain**
- **26/26 acceptance checks pass** (Hinglish parsing, voice correction, false
  positives, silence detection, safety blocks, fail-safe, 30 tools)
- Whisper model **actually download hua aur load hua** (1.8s)
- Web search **live chala** (IRCTC query pe real results)
- Self-healing **teeno levels** verified (fake device se UI change simulate karke)
- Dikha Do Mode **end-to-end** (sikhaya → replay → UI badla → khud theek hua)
- NVIDIA `/models` **live chala** (102 models, nemotron confirmed)

### ❌ NAHI Verified (yahi pehla kaam hai)
- **Asli microphone pe voice** — sandbox mein mic nahi tha
- **Asli speaker pe TTS** — sandbox mein audio output nahi tha
- **Asli phone pe ADB** — sandbox mein phone nahi tha
- **Asli LLM se pura conversation** — keys sandbox mein nahi thi

> **Naye AI/dev ke liye:** Ye ASLI HARDWARE PE TEST karna **pehla kaam** hai,
> naya feature banane se pehle. Jo bugs milenge wo asli honge.
...
- `devices/browser.py` — Playwright se `BrowserDevice`
  **Fayda:** DOM hi `ui_tree` ban jaayega, isliye `tap_text` aur **self-healing
  dono automatically kaam karenge.** Architecture ka payoff.
...
```
**`/models`** = model deprecate ho jaaye to ye chala.
...
|---|---|
| **Repo** | https://github.com/theafzalhussain/Sarthi |
| **Commits** | 6 (Phase 1, Phase 2, model fix, NVIDIA, Bluesminds) |
| **Files** | 50 tracked, 42 Python |
| **Lines** | ~12,000 Python |
| **Python** | 3.9+ |
| **Tools** | 30 |
| **Indian apps** | 110 |
| **LLM providers** | 5 |
| **ASR corrections** | 55 rules |
| **Tests** | 26/26 pass (sandbox) |
| **Phase 1** | ✅ Complete |
| **Phase 2** | ✅ Complete |
| **Phase 3-5** | ⬜ Pending |
| **Real hardware test** | ❌ Pending — **pehla kaam** |
